...
import eu.prestoprime.conf.ConfigurationManager.PropertyManager;
...
public class P4DIP extends P4InformationPackage implements DIP {
	...
	public String getPubFolder() {
		PropertyManager properties = ConfigurationManager.getPropertyInstance();
		...
		if (resources == null || resources.size() == 0)
			...
			for (String resource : resources) {
				...
				try {
					...
					Node resNode = PersistenceManager.getInstance().readXMLResource(collectionName, resId);
		...
		if (resources != null && !resources.isEmpty()) {
			...
			for (String resource : resources) {
				...
				try {
					...
					e.printStackTrace();
					throw new P4IPException("Unable to find DIP resource " + resId + " in Collection " + collectionName.getCollectionName() + "...");
				}
			}
		}
