/**
 * MakeConsumerSegmentServlet.java
...
@WebServlet("/access/consumersegment")
public class MakeConsumerSegmentServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LoggerFactory.getLogger(MakeConsumerSegmentServlet.class);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String startFrame = request.getParameter("start.frame");
		String stopFrame = request.getParameter("stop.frame");

		logger.debug("Requesting consumer copy for DIP " + id + " and mimetype " + mimetype + "...");

		// get wfID
		String wfID = "make_consumer_segment";

		// execute workflow
		Map<String, String> dParamsString = new HashMap<>();
		dParamsString.put("dipID", id);
		dParamsString.put("mimetype", mimetype);
		dParamsString.put("start.frame", startFrame);
		dParamsString.put("stop.frame", stopFrame);
