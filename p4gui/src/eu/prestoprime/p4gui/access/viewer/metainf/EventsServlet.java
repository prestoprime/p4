...
@WebServlet("/access/viewer/metainf/7")
public class EventsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			List<Event> events = AccessConnection.getEvents(user.getCurrentP4Service(), id);
			request.setAttribute("events", events);
		} catch (Throwable e) {
			e.printStackTrace();
		}
		Tools.servletInclude(this, request, response, "/body/access/viewer/metainf/events/viewEvents.jsp");
	}
}
