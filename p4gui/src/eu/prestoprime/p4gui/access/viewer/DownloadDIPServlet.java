...
@WebServlet("/access/viewer/preview")
public class DownloadDIPServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (id != null) {
				logger.debug("Retrieving DIP " + id);
				String dip = AccessConnection.getDIP(user.getCurrentP4Service(), id).getContentAsString(false);

				response.setContentType("text/xml");
				response.setContentLength(dip.length());
				response.setHeader("Content-Disposition", "attachment; filename=\"DIP.xml\"");

				response.getWriter().write(dip);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
