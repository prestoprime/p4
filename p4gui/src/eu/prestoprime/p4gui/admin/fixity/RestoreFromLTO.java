...
@WebServlet("/admin/fixity/restoreFromLTO.do")
public class RestoreFromLTO extends HttpServlet {
	private static final long serialVersionUID = 1L;

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		User user = ((User) Tools.getSessionAttribute(request.getSession(), "user", User.class));
		RoleManager.checkRequestedRole(USER_ROLE.admin, user.getCurrentP4Service().getRole(), response);

		String from = request.getParameter("from");
		String to = request.getParameter("to");

		if (AdminConnection.restoreFromLTO(user.getCurrentP4Service(), from, to)) {
			request.setAttribute("result", "success");
		} else {
			request.setAttribute("result", "error");
		}

		Tools.servletInclude(this, request, response, "/body/admin/fixity/resultRestore.jsp");
	}
}
